# SIH26045 — Neo4j Knowledge Graph Schema (Cypher DDL + Traversal Queries)

> **Purpose:** Formulation → IP → Regulatory graph for instant pathway lookup
> **Use case:** RAG retrieval augmentation — structured answers from graph traversals
...
```cypher
// ============================
// CLASSIFICATIONS (5 types per PS)
// ============================
...
// Link formulations to classifications
...
MERGE (ip5:IPRight {type: "GI", requirements: ["Geographical Origin", "Quality/Reputation Link", "Collective Rights"], term_years: -1, jurisdiction: "India"}); // -1 = indefinite
...
```cypher
// ============================
// CLASSIFICATION → IP RIGHTS (BARS / ALLOWS)
// ============================
...
// ============================
MATCH (f:Formulation {classification: "Classical"}), (p:Procedure) WHERE p.id IN ["gi-registration", "tm-filing", "cdsco-classical"] MERGE (f)-[:REQUIRES]->(p);
...
MATCH (f:Formulation {classification: "Cosmetic"}), (p:Procedure) WHERE p.id IN ["design-filing", "tm-filing", "cdsco-cosmetic"] MERGE (f)-[:REQUIRES]->(p);
...
```python
# /Users/shreyashsingh/my info/projects/sih26/prototype/neo4j_schema/init_graph.py
"""
Initialize Neo4j knowledge graph for SIH26045 IP-SAKTI
Run: python init_graph.py
...
# All the MERGE statements from above (truncated for brevity - use the full Cypher from schema.cypher)
...
def load_cypher_file(driver, filepath):
    with open(filepath, 'r') as f:
        content = f.read()
    
    # Split by semicolon (naive but works for our DDL)
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Load full schema from .cypher file
        cypher_path = os.path.join(os.path.dirname(__file__), "schema.cypher")
        if os.path.exists(cypher_path):
            load_cypher_file(driver, cypher_path)
            print("Schema loaded from schema.cypher")
...
| File | Path |
|---|---|
| Schema DDL + Data | `projects/sih26/prototype/neo4j_schema/schema.cypher` |
| Python init | `projects/sih26/prototype/neo4j_schema/init_graph.py` |
| Queries | `projects/sih26/prototype/neo4j_schema/queries.cypher` |
| This spec | `reference/research/sih26/neo4j-schema-2026-09-03.md` |

---

## Next: 
1. `neo4j-admin database create sih26` (or use Aura free tier)
2. `pip install neo4j`
3. `export NEO4J_PASSWORD=your_password`
4. `python init_graph.py`
5. Test queries in Neo4j Browser